...
- **Re-enable the Release workflow.** Do this last — the `npm-release` environment above must exist first, or the first post-enable push to `main` fails with nowhere to deploy from. It is intentionally disabled on _this template repo_ (see "Release workflow" below); a generated repo needs it on: `gh workflow enable Release`. (Template generation copies files with workflows enabled by default, so this is only a guard against the rare case where it was carried over disabled.) See [README → Enable the Release workflow](README.md#enable-the-release-workflow).
