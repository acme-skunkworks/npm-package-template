...
`ci.yml` ends with a single **`go/no-go`** aggregator job — the one stable, estate-canonical gate the release-orchestrator waits on (SK-412/SK-424). It `needs:` every real job (`config`, `lint`, `build-test`, `pr-title`, `changelog-completeness`), runs `if: ${{ always() }}`, and a one-line `jq` verdict over `toJSON(needs)` succeeds **iff** every job `result` is `success` or `skipped`. The `lint`, `build-test`, and `pr-title` jobs are thin callers of the shared reusable workflows (see "Shared reusable CI callers" below); `config` is in `needs` so a config failure — which would skip the callers, and skips are accepted — still fails the gate directly.
...
- **Footguns (SK-418).** The gate must **never** be path-filtered (a path-filtered required check sits Pending forever and blocks merges); `always()` is mandatory or the aggregator skips and never reports; the literal `/` must surface as `check_run.name == "go/no-go"` (it does — emoji/spaces already survive in `lint / Lint`). Fall back to explicit-create (`POST /check-runs`, Option A) only if the `/` ever misbehaves.
- **Transition (SK-419).** The private release-orchestrator still waits on `🔬 Build & Lint` until SK-419 flips it to the `go/no-go` check-run (dual-accept, then `go/no-go`-only). The caller swap (SK-447) **removed** the `🔬 Build & Lint` context from this template — replaced by `lint / Lint` + `build-test / Build & Test` — so the fleet rollout (SK-420) must land alongside SK-419. Safe on _this_ repo: Release is disabled, so nothing here waits on the orchestrator. Since SK-432 made `pr-title` a reusable caller, its context is `pr-title / Validate PR title is a Conventional Commit` — the estate-pinned required-check context (SK-405); the job id must stay `pr-title` (the prefix derives from it) and don't tidy the reusable job name.
- **Done (SK-411 / SK-447 / SK-432).** The `lint`, `build-test`, and `pr-title` jobs are now thin callers of `acme-skunkworks/shared-workflows`'s reusable `reusable-lint.yml` / `reusable-build-test.yml` (SK-415/416) / `reusable-validate-pr-title.yml` (SK-427). Only `release` stays inline (no reusable workflow yet, SK-417). `go/no-go` stays put across the swap, which is exactly why it lives here (custom per-repo) and not upstream.

## Shared reusable CI callers (SK-447)

The `lint`, `build-test`, and `pr-title` jobs in `ci.yml` are thin callers of the estate's shared reusable workflows — the template is the **reference consumer** that proves the pattern before the fleet rollout (SK-420). All three are SHA-pinned to one commit (`9b7e7dc`) so Dependabot bumps them together. (The interactive `claude.yml` + `claude-code-review.yml` workflows are likewise callers of the shared reusable Claude pair, SK-432, pinned to the same SHA.)

- **`lint`** → `reusable-lint.yml` runs ESLint + markdownlint + yamllint/actionlint + dated-changelog validation in one job (`lint / Lint`). Inputs: `eslint-args` passes **directory paths** (`src infrastructure/scripts infrastructure/send-it`), not globs — the Layer-1 action runs `eslint $ESLINT_ARGS` word-split with bash `globstar` off, so a `**` glob would mis-expand; directories let ESLint's flat config resolve the file set recursively. `markdown-globs` mirrors the `lint:md` script; `changelog-script: validate:changelog` (the repo's script name; the reusable default is `changelog:validate`). The yaml lane uses **actionlint 1.7.12** (the reusable default, owned upstream in lockstep per SK-422 — _not_ the `1.7.5` the local `ensure-actionlint.sh` pins) and shared-workflows' **centralised `.yamllint.yml`** (SK-438), so the repo's local `.yamllint.yml` now only feeds the pre-commit hook.
...
- **`pr-title`** → `reusable-validate-pr-title.yml` lints the PR title as a Conventional Commit (`build-test / Build & Test`'s sibling context `pr-title / Validate PR title is a Conventional Commit`). No `with:` — the reusable `types` default is the estate set `/send-it` emits (feat/fix/docs/chore/ci/refactor/perf/test/build/style/revert), which matches this repo exactly. The job id MUST stay `pr-title` so the context prefix holds (SK-405). Takes no `node-version-file`, so unlike the lint/build-test callers it does **not** `needs: config`.
- **`config`** loads `repo-config.yaml` once and feeds `node-version-file` to the `lint`/`build-test` callers (and to `changelog-completeness`) via `needs` — caller jobs can't run a `load-repo-config` step inline. This is the caller-stub pattern: a generated package that changes `nodeVersionFile` has it flow through without editing the workflow.
...
Two non-Node tools augment Prettier's formatting pass with the semantic checks Prettier can't see (Actions schema, `${{ … }}` expression typos, duplicate keys, etc.). Since SK-447, **CI runs them inside the `lint` reusable caller** (the yaml lane), not the inline jobs described here — so the install scripts below are now the **local/pre-commit + reference** path. Note the version split: the reusable workflow pins **actionlint 1.7.12** (and yamllint 1.37.1); the local `ensure-actionlint.sh` still pins **1.7.5**.
...
| `ci.yml` → `pr-title`                     | ⚠️ remote   | Since SK-432 a thin caller of `acme-skunkworks/shared-workflows`'s `reusable-validate-pr-title.yml` (was inline `amannn/action-semantic-pull-request`). `act` must **fetch the remote reusable workflow** (needs network + a `GITHUB_TOKEN`); the decisive check is the real PR run. The reusable reads the title from the PR event fixture. |
...
| `claude-code-review.yml` / `claude.yml`   | ⏭️ skip     | Since SK-432 both are thin callers of the shared reusable Claude pair (`reusable-claude.yml` / `reusable-claude-code-review.yml`). Still need `CLAUDE_CODE_OAUTH_TOKEN` (and now a remote-workflow fetch); the `act:*` scripts use `-W` to scope to specific workflows, so these aren't loaded by default.                                   |
...
The `changelog/` directory is the **only** changelog in the repo — there is no root `CHANGELOG.md` (release-please runs with `skip-changelog`, SK-371). It keeps **one dated Markdown file per shippable change** — a browsable, per-change, machine-readable record (a `version` field per entry ties it back to the published release). `release.yml` sources its GitHub-release notes from these entries. Full schema and lifecycle in **`changelog/README.md`**. The template ships only that README; the first real entry is written by `/send-it`.
...
**The publish step uses a wrapper script (no `changesets/action`).** The `🚀 Publish (npm)` step runs `bash infrastructure/scripts/publish-via-raw-npm.sh` directly, which calls `$PNPM_HOME/npm publish "$TARBALL" --access public --provenance` on the prebuilt tarball (ASW-328). Two reasons (both diagnosed in ASW-174):
...
- **Separate `publish-github-packages` job**, gated `needs: release` + `if: needs.release.outputs.should_publish == 'true'` **plus the same main-only ref guard + `npm-release` environment** (ASW-326). `packages: write` is scoped to this job only — never to the `release` job that holds `id-token: write` for npm OIDC.
