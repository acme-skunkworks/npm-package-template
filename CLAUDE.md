...
`ci.yml` ends with a single **`go/no-go`** aggregator job — the one stable, estate-canonical gate the release-orchestrator waits on (SK-412/SK-424). It `needs:` every real job (`config`, `lint`, `build-test`, `pr-title`, `changelog-completeness`), runs `if: ${{ always() }}`, and a one-line `jq` verdict over `toJSON(needs)` succeeds **iff** every job `result` is `success` or `skipped`. The `lint` and `build-test` jobs are thin callers of the shared reusable workflows (see "Shared reusable CI callers" below); `config` is in `needs` so a config failure — which would skip the callers, and skips are accepted — still fails the gate directly.
...
- **Transition (SK-419).** The private release-orchestrator still waits on `🔬 Build & Lint` until SK-419 flips it to the `go/no-go` check-run (dual-accept, then `go/no-go`-only). The caller swap (SK-447) **removed** the `🔬 Build & Lint` context from this template — replaced by `lint / Lint` + `build-test / Build & Test` — so the fleet rollout (SK-420) must land alongside SK-419. Safe on _this_ repo: Release is disabled, so nothing here waits on the orchestrator. `pr-title`'s name is _also_ the estate-pinned required-check context (SK-405); don't tidy it.
- **Done (SK-411 / SK-447).** The `lint` and `build-test` jobs are now thin callers of `acme-skunkworks/shared-workflows`'s reusable `reusable-lint.yml` / `reusable-build-test.yml` (SK-415/416). `pr-title` stays inline (its own track, SK-428/SK-403) and `release` stays inline (no reusable workflow yet, SK-417). `go/no-go` stays put across the swap, which is exactly why it lives here (custom per-repo) and not upstream.

## Shared reusable CI callers (SK-447)

The `lint` and `build-test` jobs in `ci.yml` are thin callers of the estate's shared reusable workflows — the template is the **reference consumer** that proves the pattern before the fleet rollout (SK-420). Both are SHA-pinned to one commit (`9b7e7dc`) so Dependabot bumps them together (SK-446).
