...
pnpm validate:changelog # schema-check changelog/*.md (CI: build-and-lint job)
...
- **Markdown** — `.markdownlint-cli2.jsonc` extends `@acme-skunkworks/markdownlint-config`. Pre-commit auto-fixes staged `**/*.{md,mdx}` via lint-staged (`|| true`, so it never blocks); the `build-and-lint` job in `ci.yml` enforces. (There is no root `CHANGELOG.md` to exclude — release-please runs with `skip-changelog`.)
...
`ci.yml` ends with a single **`go/no-go`** aggregator job — the one stable, estate-canonical gate the release-orchestrator waits on (SK-412/SK-424). It `needs:` every real job (`build-and-lint`, `pr-title`, `yaml-lint`, `infra`), runs `if: ${{ always() }}`, and a one-line `jq` verdict over `toJSON(needs)` succeeds **iff** every job `result` is `success` or `skipped` (the latter covers `yaml-lint`/`infra` legitimately skipping on the release-please branch).
...
- **Footguns (SK-418).** The gate must **never** be path-filtered (a path-filtered required check sits Pending forever and blocks merges); `always()` is mandatory or the aggregator skips and never reports; the literal `/` must surface as `check_run.name == "go/no-go"` (it does — emoji/spaces already survive in `🔬 Build & Lint`). Fall back to explicit-create (`POST /check-runs`, Option A) only if the `/` ever misbehaves.
- **Transition (SK-419).** The private release-orchestrator still waits on `🔬 Build & Lint` today; it flips to `go/no-go` (dual-accept, then `go/no-go`-only) under SK-419 — so the `build-and-lint` / `pr-title` job names must stay verbatim for now. `pr-title`'s name is _also_ the estate-pinned required-check context (SK-405); don't tidy it.
- **Deferred (SK-411).** The four aggregated jobs are still inline. They become thin callers of `acme-skunkworks/shared-workflows`'s reusable `lint`/`build-test`/`pr-title` workflows once that repo exists (SK-415/416/417); `go/no-go` stays put across that swap, which is exactly why it lives here (custom per-repo) and not upstream.
...
| ----------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml` → `build-and-lint`               | ✅ full     | Checkout → pnpm → Node 22 → install → build → lint → validate:changelog → changelog-completeness all green. The completeness gate is a no-op unless the PR title is `feat`/`fix`/breaking (it reads `PR_TITLE`, unset under `act`).                                                       |
...
| `scripts/check-changelog-completeness.ts` | `ci.yml` build-and-lint changelog gate              | `tests/check-changelog-completeness.test.ts` (vitest — release-triggering title + entry-presence cases)               |
...
CI: the `infra` job in `ci.yml` runs `pnpm lint:sh`, `pnpm test`, and `pnpm test:sh` against this directory; `pnpm validate:changelog` and the changelog-completeness gate run in the `build-and-lint` job (so they also cover the release-please branch's finalised entries). Locally, `pnpm lint:sh` / `pnpm test:sh` skip with install hints if `shellcheck` / `bats` aren't on PATH — `pnpm test` (vitest) always runs because vitest is a node devDep.
