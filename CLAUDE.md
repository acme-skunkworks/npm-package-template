...
The one-time org/repo settings that stand this up as a GitHub Template repository — and the settings every spawned repo inherits (Template flag, `GO/NO GO` ruleset, npm OIDC, `npm-release` environment, orchestrator onboarding, Claude review) — live in the [Setup section of the README](README.md#setup), the single source of truth for the non-copied setup. The list below covers only the per-package edits inside a generated repo; each non-copied repo/org step below cross-links its README subsection rather than duplicating it.
...
- **Reset `changelog/` to just its `README.md`.** The template dogfoods its own changelog process, so `changelog/` accumulates dated entries documenting the _template's own_ development. "Use this template" copies them into the generated repo, where they are unrelated noise — and because they are version-less, `finalise-changelog` would stamp the new package's first version onto them and sweep them into its first release notes (it stamps _every_ version-less entry, not just the current PR's). Delete every dated entry, keeping only `README.md`: `git rm changelog/*-*.md` (or remove all `changelog/*.md` except `README.md`). Do this before the first release; the template's entries are preserved in its own git history. The repo's own first real entry is written by `/send-it`.
...
- **Re-enable the Release workflow.** It is intentionally disabled on _this template repo_ (see "Release workflow" below); a generated repo needs it on: `gh workflow enable Release`. (Template generation copies files with workflows enabled by default, so this is only a guard against the rare case where it was carried over disabled.) See [README → Enable the Release workflow](README.md#enable-the-release-workflow).
- **Re-create the `GO/NO GO` required-check ruleset.** Branch protection and rulesets are **not** copied by "Use this template", so a generated repo must add its own ruleset requiring the `GO/NO GO` check-run on the default branch, pinned to the GitHub Actions integration (see "CI gate (`GO/NO GO`)" below). Without it the gate runs but nothing enforces it. See [README → the required-check ruleset](README.md#the-required-check-ruleset).
- **Create the `npm-release` environment.** Not copied by "Use this template". Both privileged release jobs deploy from a `main`-restricted `npm-release` environment; without it the OIDC publish has nowhere to deploy from and the release fails. See [README → the npm-release environment](README.md#the-npm-release-environment).
- **Onboard the release-orchestrator.** The template already ships every repo-side prerequisite (release-please config/manifest, `changelog:finalise`, `.nvmrc`, publish-only `pkg-release.yml`, `GO/NO GO` on `release-please--*`), so onboarding reduces to **install road-runner-bot + add the repo to the orchestrator's `matrix.repo`** (A-648). Without it the repo never gets its hands-off release PRs. See [README → release-orchestrator onboarding](README.md#release-orchestrator-onboarding).
- **Verify the Claude review prerequisites.** `claude.yml` / `claude-code-review.yml` need the `CLAUDE_CODE_OAUTH_TOKEN` secret **and** the Claude GitHub App installed on the repo — neither is copied, and the App install is the half that fixes the `git fetch … could not read Username` failure (A-621 / A-636). Preferably both are provisioned org-wide, in which case just confirm the repo inherits them. See [README → Claude review prerequisites](README.md#claude-review-prerequisites).
