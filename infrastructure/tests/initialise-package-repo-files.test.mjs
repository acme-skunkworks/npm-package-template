...
describe("deriveIdentity + applyIdentity", () => {
  ...
  it("rewrites the package identity block", () => {
    const pkg = {
      ...
      keywords: ["template"],
      name: "@acme-studio/npm-package-template",
      repository: {
        type: "git",
  ...
  it("recognises the placeholder name as the not-yet-renamed signal", () => {
    expect(isPlaceholderName("@acme-studio/npm-package-template")).toBe(
      true,
    );
    expect(isPlaceholderName("@acme-studio/portcullis")).toBe(false);
  });
});
